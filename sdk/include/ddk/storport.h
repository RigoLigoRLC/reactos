...
#define MAXIMUM_CDB_SIZE 12
    ...
    typedef struct _SCSI_REQUEST_BLOCK
    {
        ...
        _Field_size_bytes_(DataTransferLength) PVOID DataBuffer; // offset 18
...
#define STORAGE_REQUEST_BLOCK_VERSION_1 0x1
    ...
    typedef struct _STORAGE_REQUEST_BLOCK
    {
        ...
        _Field_range_(STORAGE_REQUEST_BLOCK_VERSION_1, STORAGE_REQUEST_BLOCK_VERSION_1) ULONG Version;
        ...
        // type address.
        //
        _Field_range_(sizeof(STORAGE_REQUEST_BLOCK), SrbLength - sizeof(STOR_ADDRESS)) ULONG AddressOffset;
...
//
// Data set management IOCTL sent to miniport driver. Lba Ranges carried by this IOCTL may cross different files or do
// not belong to file. This IOCTL carries SRB_IO_CONTROL and DEVICE_MANAGE_DATA_SET_ATTRIBUTES as part of input
// parameters. NOTE that when construct input buffer, padding place may be needed between SRB_IO_CONTROL and
// DEVICE_MANAGE_DATA_SET_ATTRIBUTES to make sure DEVICE_MANAGE_DATA_SET_ATTRIBUTES is pointer safe. e.g. input buffer
// layout should be: ALIGN_UP(sizeof(SRB_IO_CONTROL), PVOID) + DEVICE_MANAGE_DATA_SET_ATTRIBUTES.
...
#define IsSenseDataFormatValueValid(SenseInfoBuffer)                                                                   \
    ...
    _Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiGetTotalSenseByteCountIndicated(
    ...
    _Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiGetFixedSenseKeyAndCodes(
        _In_reads_bytes_(SenseInfoBufferLength) PVOID SenseInfoBuffer,
        _In_ UCHAR SenseInfoBufferLength,
    ...
    _Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiGetDescriptorSenseKeyAndCodes(
        _In_reads_bytes_(SenseInfoBufferLength) PVOID SenseInfoBuffer,
        _In_ UCHAR SenseInfoBufferLength,
        _Out_opt_ PUCHAR SenseKey,
        _Out_opt_ PUCHAR AdditionalSenseCode,
...
#define SCSI_SENSE_OPTIONS_FIXED_FORMAT_IF_UNKNOWN_FORMAT_INDICATED ((SCSI_SENSE_OPTIONS)0x00000001)
    ...
    _Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiGetSenseKeyAndCodes(
        _In_reads_bytes_(SenseInfoBufferLength) PVOID SenseInfoBuffer,
        _In_ UCHAR SenseInfoBufferLength,
    ...
    --*/
    {
        ...
        if (IsDescriptorSenseDataFormat(SenseInfoBuffer))
        {
            ...
            succeed = ScsiGetDescriptorSenseKeyAndCodes(
                SenseInfoBuffer, SenseInfoBufferLength, SenseKey, AdditionalSenseCode, AdditionalSenseCodeQualifier);
        }
        else
        {
                ...
                IsFixedSenseDataFormat(SenseInfoBuffer))
            {
                ...
                succeed = ScsiGetFixedSenseKeyAndCodes(
                    SenseInfoBuffer, SenseInfoBufferLength, SenseKey, AdditionalSenseCode,
                    AdditionalSenseCodeQualifier);
            }
        }
    ...
    _Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiGetSenseDescriptor(
        _In_reads_bytes_(SenseInfoBufferLength) PVOID SenseInfoBuffer,
        _In_ UCHAR SenseInfoBufferLength,
        ...
        *DescriptorBufferLength = 0;
        ...
        if (RTL_CONTAINS_FIELD(descriptorSenseData, SenseInfoBufferLength, AdditionalSenseLength))
        {
            ...
            if (descriptorSenseData->AdditionalSenseLength <=
                (MAX_SENSE_BUFFER_SIZE - RTL_SIZEOF_THROUGH_FIELD(DESCRIPTOR_SENSE_DATA, AdditionalSenseLength)))
            {
    ...
    _Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiValidateInformationSenseDescriptor(
    ...
    _Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiValidateBlockCommandSenseDescriptor(
        _In_reads_bytes_(DescriptorBufferLength) PVOID DescriptorBuffer,
        _In_ UCHAR DescriptorBufferLength)
    /*++
    Description:
    ...
    _Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiConvertToFixedSenseFormat(
        _In_reads_bytes_(SenseInfoBufferLength) PVOID SenseInfoBuffer,
        _In_ UCHAR SenseInfoBufferLength,
    ...
    --*/
    {
        ...
        if (IsDescriptorSenseDataFormat(SenseInfoBuffer))
        {
            ...
            validSense = ScsiGetSenseKeyAndCodes(
                SenseInfoBuffer, SenseInfoBufferLength, SCSI_SENSE_OPTIONS_NONE, &senseKey, &additionalSenseCode,
                &additionalSenseCodeQualifier);
            if (validSense)
            {
                ...
                outBuffer->AdditionalSenseLength =
                    sizeof(FIXED_SENSE_DATA) - RTL_SIZEOF_THROUGH_FIELD(FIXED_SENSE_DATA, AdditionalSenseLength);
    ...
    _Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiGetNextSenseDescriptorByType(
    ...
    --*/
    {
        ...
        if (Buffer == NULL || BufferLength == 0 || TypeList == NULL || TypeListCount == 0 || OutType == NULL ||
            OutBuffer == NULL || OutBufferLength == NULL)
        {
    ...
    typedef BOOLEAN
    HW_MESSAGE_SIGNALED_INTERRUPT_ROUTINE(IN PVOID HwDeviceExtension, IN ULONG MessageId);
...
#define STOR_MAP_NO_BUFFERS                                                                                            \
    (0) // only maps buffer for SRB_FUNCTION_IO_CONTROL, SRB_FUNCTION_PROTOCOL_COMMAND and SRB_FUNCTION_WMI
...
#define STOR_ADAPTER_FEATURE_STOP_UNIT_DURING_POWER_DOWN                                                               \
    0x00000002 // Indicating the adapter wants STOP_UNIT command during system shutdown
#define STOR_ADAPTER_UNCACHED_EXTENSION_NUMA_NODE_PREFERRED                                                            \
    0x00000004 // Indicating that miniport driver wants UncachedExtension to be allocated from adapter NUMA node.
#define STOR_ADAPTER_DMA_V3_PREFERRED                                                                                  \
    0x00000008 // Indicating that miniport driver prefers to use DMA V3 kernel API for the adapter.
#define STOR_ADAPTER_FEATURE_ABORT_COMMAND                                                                             \
    0x00000010 // Indicating the miniport driver supports the ability to abort an outstanding command via
               // SRB_FUNCTION_ABORT_COMMAND.
#define STOR_ADAPTER_FEATURE_RICH_TEMPERATURE_THRESHOLD                                                                \
    0x00000020 // Indicating the adapter supports richer temperature threshold information than defined in SCSI SPC4
               // spec.
...
#define RAID_ASYNC_NOTIFY_SUPPORTED_FLAGS                                                                              \
    (RAID_ASYNC_NOTIFY_FLAG_MEDIA_STATUS | RAID_ASYNC_NOTIFY_FLAG_DEVICE_STATUS |                                      \
...
#define MARK_DUMP_MEMORY_FLAG_PHYSICAL_ADDRESS 0x1
    ...
    typedef ULONG
    sp_DRIVER_INITIALIZE(_In_ PVOID DriverObject, _In_ PVOID RegistryPath);
    typedef sp_DRIVER_INITIALIZE *sp_PDRIVER_INITIALIZE;

    typedef BOOLEAN
    HW_INITIALIZE(_In_ PVOID DeviceExtension);
    typedef HW_INITIALIZE *PHW_INITIALIZE;

    typedef BOOLEAN
    HW_BUILDIO(_In_ PVOID DeviceExtension, _In_ PSCSI_REQUEST_BLOCK Srb);
    typedef HW_BUILDIO *PHW_BUILDIO;

    typedef BOOLEAN
    HW_STARTIO(_In_ PVOID DeviceExtension, _In_ PSCSI_REQUEST_BLOCK Srb);
    typedef HW_STARTIO *PHW_STARTIO;

    typedef BOOLEAN
    HW_INTERRUPT(_In_ PVOID DeviceExtension);
    typedef HW_INTERRUPT *PHW_INTERRUPT;

    typedef VOID
    HW_TIMER(_In_ PVOID DeviceExtension);
    typedef HW_TIMER *PHW_TIMER;

    typedef VOID
    HW_TIMER_EX(_In_ PVOID DeviceExtension, _In_opt_ PVOID Context);
    typedef HW_TIMER_EX *PHW_TIMER_EX;

    typedef VOID
    HW_DMA_STARTED(_In_ PVOID DeviceExtension);
    ...
    typedef BOOLEAN
    HW_RESET_BUS(_In_ PVOID DeviceExtension, _In_ ULONG PathId);
    typedef HW_RESET_BUS *PHW_RESET_BUS;

    typedef BOOLEAN
    HW_ADAPTER_STATE(_In_ PVOID DeviceExtension, _In_ PVOID Context, _In_ BOOLEAN SaveState);
    typedef HW_ADAPTER_STATE *PHW_ADAPTER_STATE;

    typedef SCSI_ADAPTER_CONTROL_STATUS
    HW_ADAPTER_CONTROL(_In_ PVOID DeviceExtension, _In_ SCSI_ADAPTER_CONTROL_TYPE ControlType, _In_ PVOID Parameters);
    typedef HW_ADAPTER_CONTROL *PHW_ADAPTER_CONTROL;

    typedef BOOLEAN
    HW_PASSIVE_INITIALIZE_ROUTINE(_In_ PVOID DeviceExtension);
    ...
    typedef VOID
    HW_WORKITEM(_In_ PVOID HwDeviceExtension, _In_opt_ PVOID Context, _In_ PVOID Worker);
    ...
    typedef VOID
    HW_TRACING_ENABLED(_In_ PVOID HwDeviceExtension, _In_ BOOLEAN Enabled);
    typedef HW_TRACING_ENABLED *PHW_TRACING_ENABLED;

    // Callback for unit control
    typedef SCSI_UNIT_CONTROL_STATUS
    HW_UNIT_CONTROL(_In_ PVOID DeviceExtension, _In_ SCSI_UNIT_CONTROL_TYPE ControlType, _In_ PVOID Parameters);
    typedef HW_UNIT_CONTROL *PHW_UNIT_CONTROL;

    typedef BOOLEAN (*PStorPortGetMessageInterruptInformation)(
        _In_ PVOID HwDeviceExtension,
        _In_ ULONG MessageId,
        _Out_ PMESSAGE_INTERRUPT_INFORMATION InterruptInfo);
    typedef VOID (*PStorPortPutScatterGatherList)(
        _In_ PVOID HwDeviceExtension,
    ...
    typedef GETSGSTATUS (*PStorPortBuildScatterGatherList)(
        _In_ PVOID HwDeviceExtension,
        _In_ PVOID Mdl,
    ...
    typedef VOID (*PStorPortFreePool)(
        _In_ __drv_freesMem(Mem) PVOID PMemory,
        _In_ PVOID HwDeviceExtension,
        _In_opt_ __drv_freesMem(Mem) PVOID PMdl);

    typedef __drv_allocatesMem(Mem) _Post_writable_byte_size_(NumberOfBytes) PVOID (*PStorPortAllocatePool)(
        _In_ ULONG NumberOfBytes,
        _In_ ULONG Tag,
        _In_ PVOID HwDeviceExtension,
        _Out_ _At_(*PMdl, _When_(return == 0, _Post_null_))
            _At_(*PMdl, _When_(return != 0, __drv_aliasesMem __drv_allocatesMem(Mem) _Post_notnull_)) PVOID *PMdl);

    typedef PVOID (*PStorPortGetSystemAddress)(_In_ PSCSI_REQUEST_BLOCK Srb);

    typedef ULONG (*PStorPortAcquireMSISpinLock)(_In_ PVOID HwDeviceExtension, _In_ ULONG MessageID);
    typedef VOID (*PStorPortReleaseMSISpinLock)(_In_ PVOID HwDeviceExtension, _In_ ULONG MessageID, _In_ ULONG OldIrql);

    typedef VOID (*PStorPortCompleteServiceIrp)(_In_ PVOID HwDeviceExtension, _In_ PVOID Irp);
    typedef PVOID (*PStorPortGetOriginalMdl)(_In_ PSCSI_REQUEST_BLOCK Srb);
...
#define SP_RETURN_BAD_CONFIG 3
    ...
    typedef VOID(NTAPI *PpostScaterGatherExecute)(
        _In_ PVOID *DeviceObject,
        _In_ PVOID *Irp,
        _In_ PSTOR_SCATTER_GATHER_LIST ScatterGather,
        _In_ PVOID Context);
    ...
    typedef VOID
    HW_FREE_ADAPTER_RESOURCES(_In_ PVOID DeviceExtension);
    typedef HW_FREE_ADAPTER_RESOURCES *PHW_FREE_ADAPTER_RESOURCES;

    typedef VOID
    HW_PROCESS_SERVICE_REQUEST(_In_ PVOID DeviceExtension, _In_ PVOID Irp);
    typedef HW_PROCESS_SERVICE_REQUEST *PHW_PROCESS_SERVICE_REQUEST;

    typedef VOID
    HW_COMPLETE_SERVICE_IRP(_In_ PVOID DeviceExtension);
    typedef HW_COMPLETE_SERVICE_IRP *PHW_COMPLETE_SERVICE_IRP;

    typedef VOID
    HW_INITIALIZE_TRACING(_In_ PVOID Arg1, _In_ PVOID Arg2);
    typedef HW_INITIALIZE_TRACING *PHW_INITIALIZE_TRACING;

    typedef VOID
    HW_CLEANUP_TRACING(_In_ PVOID Arg1);
...
#define STOR_FEATURE_FULL_PNP_DEVICE_CAPABILITIES                                                                      \
    0x00000004 // indicating the miniport driver fills in complete information in STOR_DEVICE_CAPABILITIES
#define STOR_FEATURE_DUMP_POINTERS 0x00000008 // indicating physical miniport driver support the dump pointers SRBs
#define STOR_FEATURE_DEVICE_NAME_NO_SUFFIX                                                                             \
    0x00000010 // indicating the miniport driver does not want the suffix "SCSI <type> Device" in device friendly name
#define STOR_FEATURE_DUMP_RESUME_CAPABLE                                                                               \
    0x00000020 // indicating the miniport driver is resume capable (i.e. dump stack functions after a hibernation
               // resume)
#define STOR_FEATURE_DEVICE_DESCRIPTOR_FROM_ATA_INFO_VPD                                                               \
    0x00000040 // indicating that port driver forms STORAGE_DEVICE_DESCRIPTOR from ATA Information VPD page rather than
               // INQUIRY data
#define STOR_FEATURE_EXTRA_IO_INFORMATION                                                                              \
    0x00000080 // Indicating that miniport driver wants SRBEX_DATA_IO_INFO in a SRBEX if available
#define STOR_FEATURE_ADAPTER_CONTROL_PRE_FINDADAPTER                                                                   \
    0x00000100 // Indicating that miniport driver can safely process AdapterControl call from Storport before receiving
               // HwFindAdapter.
#define STOR_FEATURE_ADAPTER_NOT_REQUIRE_IO_PORT                                                                       \
    0x00000200 // Indicating that miniport driver doesn't require IO Port resource for its adapter.
#define STOR_FEATURE_DUMP_16_BYTE_ALIGNMENT                                                                            \
    0x00000400 // Indicating that the miniport driver wants its HwDeviceExtension to be 16 byte aligned in dump mode
#define STOR_FEATURE_SET_ADAPTER_INTERFACE_TYPE                                                                        \
    0x00000800 // Indicating that the miniport driver wants storport to set the adapter interface type.
...
#define STOR_CURRENT_LOG_INTERFACE_REVISION 0x0100
    ...
    typedef BOOLEAN
    STOR_SYNCHRONIZED_ACCESS(_In_ PVOID HwDeviceExtension, _In_ PVOID Context);
...
#define StorPortCopyMemory(Destination, Source, Length) memcpy((Destination), (Source), (Length))
    ...
    BOOLEAN
    NTAPI
    StorPortDeviceReady(_In_ PVOID HwDeviceExtension, _In_ UCHAR PathId, _In_ UCHAR TargetId, _In_ UCHAR Lun);

    STORPORT_API
    VOID NTAPI
    StorPortFreeDeviceBase(_In_ PVOID HwDeviceExtension, _In_ PVOID MappedAddress);

    STORPORT_API
    VOID NTAPI
    StorPortFreeRegistryBuffer(_In_ PVOID HwDeviceExtension, _In_ PUCHAR Buffer);
    ...
    PVOID
    NTAPI
    StorPortGetLogicalUnit(_In_ PVOID HwDeviceExtension, _In_ UCHAR PathId, _In_ UCHAR TargetId, _In_ UCHAR Lun);
    ...
    PVOID
    NTAPI
    StorPortGetVirtualAddress(_In_ PVOID HwDeviceExtension, _In_ STOR_PHYSICAL_ADDRESS PhysicalAddress);
    ...
    STORPORT_API
    VOID __cdecl StorPortNotification(_In_ SCSI_NOTIFICATION_TYPE NotificationType, _In_ PVOID HwDeviceExtension, ...);
    ...
    STORPORT_API
    VOID NTAPI
    StorPortReadPortBufferUchar(_In_ PVOID HwDeviceExtension, _In_ PUCHAR Port, _In_ PUCHAR Buffer, _In_ ULONG Count);

    STORPORT_API
    VOID NTAPI
    StorPortReadPortBufferUlong(_In_ PVOID HwDeviceExtension, _In_ PULONG Port, _In_ PULONG Buffer, _In_ ULONG Count);
    ...
    BOOLEAN
    NTAPI
    StorPortResumeDevice(_In_ PVOID HwDeviceExtension, _In_ UCHAR PathId, _In_ UCHAR TargetId, _In_ UCHAR Lun);
    ...
    STORPORT_API
    VOID NTAPI
    StorPortWritePortBufferUchar(_In_ PVOID HwDeviceExtension, _In_ PUCHAR Port, _In_ PUCHAR Buffer, _In_ ULONG Count);

    STORPORT_API
    VOID NTAPI
    StorPortWritePortBufferUlong(_In_ PVOID HwDeviceExtension, _In_ PULONG Port, _In_ PULONG Buffer, _In_ ULONG Count);
    ...
    STORPORT_API
    VOID NTAPI
    StorPortWritePortUchar(_In_ PVOID HwDeviceExtension, _In_ PUCHAR Port, _In_ UCHAR Value);

    STORPORT_API
    VOID NTAPI
    StorPortWritePortUlong(_In_ PVOID HwDeviceExtension, _In_ PULONG Port, _In_ ULONG Value);

    STORPORT_API
    VOID NTAPI
    StorPortWritePortUshort(_In_ PVOID HwDeviceExtension, _In_ PUSHORT Port, _In_ USHORT Value);
    ...
    STORPORT_API
    VOID NTAPI
    StorPortWriteRegisterUchar(_In_ PVOID HwDeviceExtension, _In_ PUCHAR Register, _In_ UCHAR Value);

    STORPORT_API
    VOID NTAPI
    StorPortWriteRegisterUlong(_In_ PVOID HwDeviceExtension, _In_ PULONG Register, _In_ ULONG Value);

    STORPORT_API
    VOID NTAPI
    StorPortWriteRegisterUshort(_In_ PVOID HwDeviceExtension, _In_ PUSHORT Register, _In_ USHORT Value);
        ...
        _In_ PHW_PASSIVE_INITIALIZE_ROUTINE HwPassiveInitializeRoutine)
    {
        LONG Succ;
        Succ = FALSE;
        StorPortNotification(EnablePassiveInitialization, DeviceExtension, HwPassiveInitializeRoutine, &Succ);
        return (BOOLEAN)Succ;
    }

    FORCEINLINE
    VOID
    StorPortInitializeDpc(_In_ PVOID DeviceExtension, _Out_ PSTOR_DPC Dpc, _In_ PHW_DPC_ROUTINE HwDpcRoutine)
    {
        ...
        _In_ PVOID SystemArgument2)
    {
        LONG Succ;
        Succ = FALSE;
        StorPortNotification(IssueDpc, DeviceExtension, Dpc, SystemArgument1, SystemArgument2, &Succ);
        return (BOOLEAN)Succ;
    }
    ...
    {
        StorPortNotification(AcquireSpinLock, DeviceExtension, SpinLock, LockContext, LockHandle);
    }

    FORCEINLINE
    VOID
    StorPortReleaseSpinLock(_In_ PVOID DeviceExtension, _Inout_ PSTOR_LOCK_HANDLE LockHandle)
    {
    ...
    STORPORT_API
    ULONG
    StorPortExtendedFunction(_In_ STORPORT_FUNCTION_CODE FunctionCode, _In_ PVOID HwDeviceExtension, ...);
    ...
    {
        return StorPortExtendedFunction(ExtFunctionAllocatePool, HwDeviceExtension, NumberOfBytes, Tag, BufferPointer);
    }
    ...
    {
        return StorPortExtendedFunction(ExtFunctionFreePool, HwDeviceExtension, BufferPointer);
    }
    ...
    StorPortAllocateMdl(
        ...
        _Out_ PVOID *Mdl)
    {
        return StorPortExtendedFunction(ExtFunctionAllocateMdl, HwDeviceExtension, BufferPointer, NumberOfBytes, Mdl);
    }
    ...
    {
        return StorPortExtendedFunction(ExtFunctionBuildMdlForNonPagedPool, HwDeviceExtension, Mdl);
    }

    FORCEINLINE
    ULONG
    StorPortGetSystemAddress(_In_ PVOID HwDeviceExtension, _In_ PSCSI_REQUEST_BLOCK Srb, _Out_ PVOID *SystemAddress)
    {
    ...
    FORCEINLINE
    ULONG
    StorPortGetOriginalMdl(_In_ PVOID HwDeviceExtension, _In_ PSCSI_REQUEST_BLOCK Srb, _Out_ PVOID *Mdl)
    {
        return StorPortExtendedFunction(ExtFunctionGetOriginalMdl, HwDeviceExtension, Srb, Mdl);
    }
    ...
    {
        return StorPortExtendedFunction(ExtFunctionCompleteServiceIrp, HwDeviceExtension, Irp);
    }
        ...
        _Out_ PVOID *LowerDeviceObject)
    {
        return StorPortExtendedFunction(
            ExtFunctionGetDeviceObjects, HwDeviceExtension, AdapterDeviceObject, PhysicalDeviceObject,
            LowerDeviceObject);
    }
    ...
    {
        return StorPortExtendedFunction(
            ExtFunctionBuildScatterGatherList, HwDeviceExtension, Mdl, CurrentVa, Length, ExecutionRoutine, Context,
            WriteToDevice, ScatterGatherBuffer, ScatterGatherBufferLength);
    }
    ...
    {
        return StorPortExtendedFunction(
            ExtFunctionPutScatterGatherList, HwDeviceExtension, ScatterGatherList, WriteToDevice);
    }

    FORCEINLINE
    ULONG
    StorPortAcquireMSISpinLock(_In_ PVOID HwDeviceExtension, _In_ ULONG MessageId, _In_ PULONG OldIrql)
    {
        return StorPortExtendedFunction(ExtFunctionAcquireMSISpinLock, HwDeviceExtension, MessageId, OldIrql);
    }

    FORCEINLINE
    ULONG
    StorPortReleaseMSISpinLock(_In_ PVOID HwDeviceExtension, _In_ ULONG MessageId, _In_ ULONG OldIrql)
    {
        return StorPortExtendedFunction(ExtFunctionReleaseMSISpinLock, HwDeviceExtension, MessageId, OldIrql);
    }
    ...
    {
        return StorPortExtendedFunction(
            ExtFunctionGetMessageInterruptInformation, HwDeviceExtension, MessageId, InterruptInfo);
    }
    ...
    {
        return StorPortExtendedFunction(
            ExtFunctionInitializePerformanceOptimizations, HwDeviceExtension, Query, PerfConfigData);
    }
    ...
    {
        return StorPortExtendedFunction(
            ExtFunctionGetStartIoPerformanceParameters, HwDeviceExtension, Srb, StartIoPerfParams);
    }
    ...
    {
        return StorPortExtendedFunction(ExtFunctionLogSystemEvent, HwDeviceExtension, LogDetails, MaximumSize);
    }
#if 0
ULONG
FORCEINLINE
    ...
    )
{
    ...
    _IRQL_requires_max_(DISPATCH_LEVEL) _Success_(return == STOR_STATUS_SUCCESS) FORCEINLINE ULONG
            ...
            _Out_ PPHYSICAL_ADDRESS PhysicalAddress)
    {
        ...
        status = StorPortExtendedFunction(
            ExtFunctionAllocateDmaMemory, HwDeviceExtension, NumberOfBytes, LowestAcceptableAddress,
            HighestAcceptableAddress, BoundaryAddressMultiple, CacheType, PreferredNode, BufferPointer,
            PhysicalAddress);
#else
    ...
    _Success_(return == STOR_STATUS_SUCCESS) FORCEINLINE ULONG StorPortFreeDmaMemory(
        _In_ PVOID HwDeviceExtension,
        ...
        _In_opt_ PHYSICAL_ADDRESS PhysicalAddress)
    {
        ...
        status = StorPortExtendedFunction(
            ExtFunctionFreeDmaMemory, HwDeviceExtension, BaseAddress, NumberOfBytes, CacheType, PhysicalAddress);
#else
    ...
    _IRQL_requires_max_(DISPATCH_LEVEL) FORCEINLINE ULONG StorPortPoFxActivateComponent(
        _In_ PVOID HwDeviceExtension,
        ...
        _In_ ULONG Flags)
    {
        ...
        status = StorPortExtendedFunction(
            ExtFunctionPoFxActivateComponent, HwDeviceExtension, Address, Srb, Component, Flags);
#else
    UNREFERENCED_PARAMETER(HwDeviceExtension);
    UNREFERENCED_PARAMETER(Address);
        ...
        _In_ ULONG Flags)
    {
        ...
        status =
            StorPortExtendedFunction(ExtFunctionPoFxIdleComponent, HwDeviceExtension, Address, Srb, Component, Flags);
#else
    UNREFERENCED_PARAMETER(HwDeviceExtension);
    UNREFERENCED_PARAMETER(Address);
    UNREFERENCED_PARAMETER(Srb);
